...
#Neural Network Methods. For this assignment, you must use a neural-network-based method such as a one-dimensional 
#convolutional neural network (1D CNN), recurrent neural network (RNN), 
#a long short-term memory (LSTM) model, or a gated recurrent unit (GRU). 
#Generative language models are also good candidates for this assignment. 

#Word embeddings are an important part of contemporary natural language processing applications. 
#Consider using word embeddings (perhaps comparing them with simple one-hot encodings) in one or 
#another of the many NLP applications. 

#Generally speaking, an experiment comparing the performance of alternative neural network language models 
#or techniques would be a good research topic for this assignment. 
...
#3. Comparisons across alternative settings for dropout regularization (none, 5 percent, . . . , 50 percent) 
#   may also be useful.

#When making comparisons across models, employ training best practices, 
#including a tripartite splitting of the corpus into training, validation (development/dev), 
#and hold-out test sets and early stopping. Ensure that meaningful indices of model 
#performance are selected, such as the area under the ROC curve for binary classification 
#and F1 for multinomial classification. 
#Additional criteria for comparing methods should include training time considerations, 
#including the number of epochs to meet an early stopping rule, the training time per epoch, 
#and the total training time.  

#Methods should be fully documented by showing the structure of each model 
#and the number of parameters being estimated. Utilize visualizations of loss 
#and accuracy across epochs for training and validation (dev) data. 
#Provide summary tables showing training, validation (dev), and hold-out test set performance, 
#as well as training time data.

#Please draw on Python/TensorFlow/Keras and the rich domain of NLP tools 
#(such as Scikit Learn, NLTK, gensim, spaCy) for your work. 
#Note that Keras provides convenient utilities for reporting on model structure, 
#early stopping, and monitoring the training process.
